...
describe 'Token verify' do
  ...
  it 'jwt with wrong combination should 404' do
    get "/verify?jwt=#{invalid_jwt}", headers: service_headers

    expect(response.code).to eq('404')
  end

  it 'jwt for retracted token should 404' do
    get "/verify?jwt=#{retracted_jwt}", headers: service_headers

    expect(response.code).to eq('404')
  end

  it 'jwt for expired token should 404' do
    get "/verify?jwt=#{expired_jwt}", headers: service_headers

    expect(response.code).to eq('404')
  end

  it 'valid jwt should 200' do
    get "/verify?jwt=#{valid_jwt}", headers: service_headers
